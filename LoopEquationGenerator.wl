...
(*Begin["`Private`"];*)
(**)
(*extractIndices[term_]:=Module[{factors,mFactors,subs},*)
(*factors=List@@term;*)
(*mFactors=Select[factors,!FreeQ[#,Subscript[M,_]]&];*)
...
(*];*)
(**)
(*extractAllIndices[expr_]:=Module[{rawIndices},*)
(*rawIndices=DeleteDuplicates[Map[extractIndices,List@@Expand[expr]]];*)
...
(*Reverse@Sort[DeleteDuplicates[Flatten[Map[extractAllIndices,derivatives],1]]]*)
(*];*)
(**)
...
(*length=Length[word];*)
(*If[length==0,{},*)
(*symmetricWordList=Flatten[Table[\!\(\*SubscriptBox[\(symmetricWord\), \({i, j}\)]\),{i,1,2},{j,1,maxLevel}]];*)
(*\!\(\*SubscriptBox[\(symmetricWord\), \({1, 1}\)]\)=word;*)
(*For[i=2,i<=length,i++,\!\(\*SubscriptBox[\(symmetricWord\), \({1, i}\)]\)=RotateRight[\!\(\*SubscriptBox[\(symmetricWord\), \({1, \((i - 1)\)}\)]\)]];*)
(*\!\(\*SubscriptBox[\(symmetricWord\), \({2, 1}\)]\)=Reverse[word];*)
(*For[i=2,i<=length,i++,\!\(\*SubscriptBox[\(symmetricWord\), \({2, i}\)]\)=RotateRight[\!\(\*SubscriptBox[\(symmetricWord\), \({2, \((i - 1)\)}\)]\)]];*)
(*Which@@Flatten[Table[{MemberQ[correlatorWords[length],\!\(\*SubscriptBox[\(symmetricWord\), \({i, j}\)]\)],\!\(\*SubscriptBox[\(symmetricWord\), \({i, j}\)]\)},{i,1,2},{j,1,length}],2]*)
(*]];*)
(**)
